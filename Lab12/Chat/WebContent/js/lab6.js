...
function init(){
	...
	if(window.WebSocket){
		//建立websock连接
		ws = new WebSocket(encodeURI('ws://localhost:8080/Chat/websocket/'+encodeURIComponent(username)));
		...
		ws.onmessage = function(event){
			...
			if(type == 'news'){
				...
				itemHtml = produceMessageHtml(message.username, message.content, message.time);
			}
...
function produceMessageHtml(username, content, date){
	...
	ret += "<div class='blog-content'> <span class='blog-name'>"+username+"</span> : ";
